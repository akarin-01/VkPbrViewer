...
namespace Kita::Pbrv
{
    namespace Resource
    {
        ...
        DescriptorAllocator::DescriptorAllocator(const Rhi::RenderContext& context, const VkDescriptorPoolCreateInfo& createInfo)
        ...
        VkDescriptorSet DescriptorAllocator::Allocate(VkDescriptorSetLayout layout, const char* debugName) const
        {
            ...
            {
                Core::Log::Error("[Resources] Descriptor pool exhausted: allocated ", m_usedSetCount, " / ",
            ...
            {
                Core::Log::Error("Failed to allocate descriptor set: ", debugName, " (", ToString(result), ")");
