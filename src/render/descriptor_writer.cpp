...
namespace Kita::Pbrv
{
    DescriptorWriter::DescriptorWriter(const RenderResources& resources, VkDevice device)
    ...
    DescriptorWriter& DescriptorWriter::WriteBuffer(uint32_t binding, VkDescriptorType type, RenderBufferHandle handle, VkDeviceSize offset, VkDeviceSize range)
    {
    ...
    DescriptorWriter& DescriptorWriter::WriteImage(uint32_t binding, VkDescriptorType type, VkImageLayout layout, RenderImageViewHandle imageViewHandle, RenderSamplerHandle samplerHandle)
    {
        BindingEntry& entry = GetOrAddEntry(binding, type);
    ...
    VkDescriptorBufferInfo DescriptorWriter::CreateBufferInfo(RenderBufferHandle handle, VkDeviceSize offset, VkDeviceSize range) const
    {
        RenderBuffer* buffer = m_resources.GetBuffer(handle);
        if (!buffer)
        {
    ...
    VkDescriptorImageInfo DescriptorWriter::CreateImageInfo(VkImageLayout layout, RenderImageViewHandle imageViewHandle, RenderSamplerHandle samplerHandle) const
    {
        RenderImageView* imageView = m_resources.GetImageView(imageViewHandle);
        if (!imageView)
        {
        ...
        if (samplerHandle != 0)
        {
            RenderSampler* sampler = m_resources.GetSampler(samplerHandle);
            if (!sampler)
            {
